"""
Greedy strategy solver for two feedback types.

Run this file directly to test the greedy strategy on both feedback types.
Defaults to length=3 for quick visible output; change length to 4 for the full run.
"""
import time
from core import all_codes, classic_feedback, positional_feedback

def run_greedy_one_secret(feedback_fn, codes, secret):
    """Solve a single secret using a greedy selection (minimize largest partition)."""
    ...
    while True:
        ...
        if len(candidates) == 1:
            ...
            best_guess = None
            best_score = None  # lower is better: size of largest partition
            ...
            for possible_guess in evaluation_pool:
                ...
                score = max(partition.values())  # greedy: minimize this
...
def test_greedy_strategy(length=3):
    ...
    print(f"Greedy strategy: testing all {total} codes (length={length})")
    print("="*60)
    ...
    for fb_name, fb_fn in feedback_types:
        ...
        for i, secret in enumerate(codes):
            ...
            steps = run_greedy_one_secret(fb_fn, codes, secret)
            counts.append(steps)
